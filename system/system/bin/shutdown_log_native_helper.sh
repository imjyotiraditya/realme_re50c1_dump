...
function print_dir_filelist()
{
    ...
    for val in ${list[*]}
    do
        echo "${val} modiy time: $(stat -c %Y ${dir}/${val})" > /dev/kmsg
    done
}
...
function collect_android_log()
{
    ...
    echo "collect android log end\n" > /dev/kmsg
}

function back_shutdown_log()
{
    echo "back_shutdown_log\n" > /dev/kmsg
    echo "TIME_BEGIN_TO_COLLECT_LOG=${TIME_BEGIN_TO_COLLECT_LOG}" > /dev/kmsg
    ...
    echo "back_shutdown_log end\n" > /dev/kmsg
}

function shd_log_native_helper_main()
{
    argv=$1
    echo "argument $argv" > /dev/kmsg
